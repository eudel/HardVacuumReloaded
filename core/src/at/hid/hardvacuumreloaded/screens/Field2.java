...
public class Field2 extends Window {
	...
	private void initialize() {
		...
		add(contentTable = new Table(skin)).width(300).height(600);
		...
		contentTable.add().colspan(3).width(300).height(400).row();
	...
	public Field2 show(Stage stage, float x, float y) {
		clearActions();
	...
	public void setField1Red(boolean red) {
		if (red == true) {
			if (isField2Red()) {
				this.setBackground("field2.red12");
			} else {
				this.setBackground("field2.red1");
			}
		} else {
			if (isField2Red()) {
				this.setBackground("field2.red2");
			} else {
				this.setBackground("field2.red");
			}
		}
		field1Red = red;
	}
